...
cvrx_poisson_l1_offset<-function(l1_norm_seq,x_basis_poisson,offset,weights,y,penalty_factor,time_vec,uniform_ratio,length_clever,coef_initial){
  ...
  if(!uniform_ratio){
    loss<-sum(exp(offset+fixed_part+X_var%*%epsilon+beta_0)*weights*(1-y))-sum((fixed_part+X_var%*%epsilon+beta_0)*y*weights)*0.00001
  }
  else{
    loss<-sum(exp(offset+fixed_part+X_var%*%epsilon+beta_0+log(1/(1-time_vec)))*weights*(1-y))-sum((fixed_part+X_var%*%epsilon+beta_0+log(1/(1-time_vec)))*y*weights)*0.00001
  }

  #Set a upper constraint for the discretization to be bounded in [0,1]
  mat_temp<-matrix(1,nrow=(p),ncol=(p))
  mat_temp[lower.tri(mat_temp)]<-0

  results <- lapply(l1_norm_seq,
                    function (l1_norm) {
                      constraint1<-sum(abs(epsilon))<= l1_norm
                      constraint2<-(mat_temp%*%beta+beta_0)<=rep(log(1/0.00001),p)
                      constraint3<-beta_0<=log(1/0.00001)
                      #constraint1<-sum((epsilon)^2)<= l1_norm
...
cvrx_poisson_l1<-function(l1_norm_seq,x_basis_poisson,offset,weights,y,penalty_factor,time_vec,uniform_ratio){
  ...
  beta <- Variable(p)
  if(!uniform_ratio){
    loss<-sum(exp(offset+X%*% beta+beta_0)*weights*(1-y))-sum((X%*% beta+beta_0)*y*weights)*0.00001
  }
  else{
    loss<-sum(exp(offset+X%*% beta+beta_0+log(1/(1-time_vec)))*weights*(1-y))-sum((X%*% beta+beta_0+log(1/(1-time_vec)))*y*weights)*0.00001
  }
